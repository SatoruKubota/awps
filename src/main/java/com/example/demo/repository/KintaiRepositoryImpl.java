...
public class KintaiRepositoryImpl implements KintaiRepository {
	...
	public List<Kintai> selectByUserIdAndDate(Integer userId, Date date) {
		...
		for (Map<String, Object> one : list) { // データベースの値を取得
			...
			Date sqlDate = (Date) one.get("DATE");
			LocalDate localDate = sqlDate.toLocalDate();
			kintai.setDate(localDate);
...
//			kintai.setStart((LocalTime) one.get("START"));						// 出勤時刻		
			Time sqlStart = (Time) one.get("START");
			LocalTime localStart = sqlStart.toLocalTime();
			kintai.setStart(localStart);
		
			
//			kintai.setFinish((LocalTime) one.get("FINISH"));					// 退勤時刻
			Time sqlFinish = (Time) one.get("FINISH");
			LocalTime localFinish = sqlFinish.toLocalTime();
			kintai.setFinish(localFinish);
	...
	public void add(Kintai kintai) {
		...
		String tableName = "kintaitable_" + kintai.getUserId();

		String sql = 
				" INSERT INTO " + 
				tableName +
				" (DATE, ATTEND_FLAG, TF_ATTEND_FLAG, SUB_HOLIDAY_FLAG, WD_FLAG, START, FINISH, WORKING_TIME, " +
				" BREAK_TIME, OT, PV_FLAG, PV_GET, SUB_HOLIDAY_GET) " +
				" VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ";	
						
		jdbcTemplate.update(sql,
				kintai.getDate(),
				kintai.getAttendFlag(),
				kintai.getTfAttendFlag(),
				kintai.getSubHolidayFlag(),
				kintai.getWdFlag(),
				kintai.getStart(),
				kintai.getFinish(),
				kintai.getWorkingTime(),
				kintai.getBreakTime(),
				kintai.getOt(),
				kintai.getPvFlag(),
				kintai.getPvGet(),
				kintai.getSubHolidayGet()
				);
